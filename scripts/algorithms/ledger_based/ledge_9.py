...
Decision making considers multi arm bandit adaptation with no prior - UCB1 (implemented probabilistic version)
'''
...
class LEDGE:
    ...
    def callback_ready(self, req):
        algo_name = req.algo
        if algo_name == 'ledge_9' and self.ready:
            return AlgoReadyResponse(True)
        else:
            return AlgoReadyResponse(False)

if __name__ == '__main__':
    rospy.init_node('ledge_9', anonymous= True)
