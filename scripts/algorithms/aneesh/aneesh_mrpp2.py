...
class CR_MRPP:
    ...
    def __init__(self, graph, num_bots):
        ...
        for i in range(self.num_bots):
            ...
            for n in self.nodes:
                ...
                for m in self.nodes:
                    self.reward_coeff['bot_{}'.format(i)][n][m] = 1.
    ...
    def callback_idle(self, data):
        if self.stamp < data.stamp:
            ...
            #callback_idle is always called before bot_next_task
            for i, n in enumerate(data.robot_id):
                if self.idle_true[data.node_id[i]] != 0. :
                    ...
                    if self.old_node[n] !=0. :
                        ...
                        if expect > true:
                            ...
                            self.reward_coeff[n][self.old_node[n]][self.current_node[n]] = 1.
    ...
    def callback_ready(self, req):
        algo_name = req.algo
        if algo_name == 'aneesh_mrpp2' and self.ready:
