...
ses                          +       reinforcement learning         +           change in value_exp
(ses for expected idleness)            (value function)              (adding the effect of high expected idleness)
AN_MRPP
epsilon-greedy
...
class AN_MRPP:
                ...
                with open('{}.csv'.format(n), 'a+', newline='') as file:
                    ...
                    if self.old_node[n] is not None :
                        ...
                        for m in self.graph.successors(self.old_node[n]):
                            ...
                            summ += math.exp(self.value_func[n][self.old_node[n]][m]) * self.idle_expect[n][self.old_node[n]][m]

                        for m in self.graph.successors(self.old_node[n]):
                            self.value_exp[n][self.old_node[n]][m] = (math.exp(self.value_func[n][self.old_node[n]][m]) * self.idle_expect[n][self.old_node[n]][m])/summ
    ...
    def callback_ready(self, req):
        algo_name = req.algo
        if algo_name == '11_ses_exp' and self.ready:
