...
ses                         +       reinforcement learning [but reward = true idleness]     + change in decision making
(ses for expected idleness)            (value function)
AN_MRPP
epsilon-greedy
...
class AN_MRPP:
                ...
                with open(name, 'a+', newline='') as file:
                    ...
                    if self.old_node[n] is not None :
                        ...
                        for m in self.graph.successors(self.old_node[n]):
                            summ += self.value_func[n][self.old_node[n]][m]

                        for m in self.graph.successors(self.old_node[n]):
                            self.value_exp[n][self.old_node[n]][m] = (self.value_func[n][self.old_node[n]][m])/summ
        ...
        if rand > epsilon:
            ...
            for n in neigh:
                ...
                for m in list(self.graph.successors(n)):
                    if self.value_exp[bot][n][m] > maxQ :
                        maxQ = self.value_exp[bot][n][m]

                #Decision making using max( expected idleness + max(value_exp of neighbour) )
                idles.append(self.idle_expect[bot][node][n] + maxQ)
    ...
    def callback_ready(self, req):
        algo_name = req.algo
        if algo_name == '12_ses_rl_true' and self.ready:
