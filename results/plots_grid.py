...
g = nx.read_graphml(rospkg.RosPack().get_path('mrpp_sumo') + '/graph_ml/grid_5_5.graphml')
...
for i,file in enumerate(sorted(fileNames)):
    ...
    new_df = df[(df['old node'] == 7) | (df['old node'] == 13) | (df['old node'] == 19)]
    ...
    for j,elem in enumerate(sorted(new_df['old node'].unique())):
        ...
        for elem in sorted((new_df2['edge'].unique())):
            ...
            ax.scatter(new_df3["time"], new_df3["probability (value_exp)"])
            ax.legend(sorted(new_df2['edge'].unique()),loc = 'upper right')


        plt.xlabel('Time (s)')
        plt.ylabel('value_exp'+ ' ' + '('+file.replace('.csv', '')+')')
        ax.figure.savefig('value_exp',bbox_inches= "tight")
...
for n,file in enumerate(sorted(fileNames)):                                          #for all nodes and 3 bots
    ax3 = plt.subplot(1, len(fileNames) , n + 1)
    df = pd.read_csv(PATH + '/'+ file)
    plt.xlabel('Time (s)')
    ...
    for elem in sorted(new_df['edge'].unique()):
        ...
        ax3.scatter(new_df2["time"], new_df2["expect"])

        ax3.legend(sorted(new_df['edge'].unique()),loc = 'upper right')
        ax3.figure.savefig('Expect_Idle_all',bbox_inches= "tight")
